/**
 * AI Program Manager Hub — Backend Server
 * Run: node server.js
...
const STATE = {
  ...
  summaries: [
    { team:'Alpha',   date:'Today 10:00 AM',    type:'Stand-up',       risk:'low',
      summary:'Team confirmed concept locked: explainable AI classifier using LIME. Dev #012 completed feature extraction. Dev #001 working on visualization layer — 60% done. Dev #045 handling test suite. No blockers. On track for Sunday demo.',
      decisions:['Use LIME for explainability','Demo at 2pm Sunday','Dev #001 owns visualization'] },
    { team:'Epsilon', date:'Yesterday 9:00 AM', type:'Sprint check-in', risk:'low',
      summary:'Monitoring service API v2 merged. Agreed on 3σ rule for anomaly detection. Dev #082 flagged a latency issue in the logging pipeline; assigned to self with 48h window. Team morale high.',
      decisions:['3σ anomaly threshold','Dev #082 owns latency fix','Next check-in Thursday'] },
    { team:'Delta',   date:'Yesterday 9:00 AM', type:'Stand-up',       risk:'high',
      summary:'Task T-14 (federated node setup) partially unassigned — Dev #047 absent. Team agreed Dev #006 covers temporarily. Concern raised about timeline slip if Dev #047 remains unreachable. Escalated to PM.',
      decisions:['Dev #006 covers T-14 temporarily','Escalate Dev #047 absence to PM'] },
  ],
...
// Simulate live anomaly detection every 45s
setInterval(() => {
  const team = STATE.teams[Math.floor(Math.random() * STATE.teams.length)];
  if (team.status === 'at-risk') {
    broadcast('anomaly-ping', { team: team.id, msg: `Anomaly monitor re-scanned Team ${team.id} — status unchanged.` });
  }
}, 45000);
...
function router(req, res) {
  ...
  // ── POST /api/ai ── general AI question
  ...
  // ── POST /api/agenda ── schedule meeting + draft agenda
  if (pathname === '/api/agenda' && req.method === 'POST') {
    body().then(async ({ target, kind, context }) => {
      ...
      const meeting = { id: `m${Date.now()}`, target, kind, agenda, scheduledAt: new Date().toISOString() };
  ...
  // ── POST /api/discord-match ── match a new member to a team
  ...
  // ── POST /api/checkin-summary ── AI summarizes a completed check-in
  ...
  // ── POST /api/simulate-checkin-miss ── demo: mark a member as missed
  if (pathname === '/api/simulate-checkin-miss' && req.method === 'POST') {
    body().then(({ team, member }) => {
      const t = STATE.teams.find(x => x.id === team);
      if (t && member && !t.missedCheckins.includes(member)) {
        t.missedCheckins.push(member);
        if (t.missedCheckins.length >= 2) t.status = 'at-risk';
      }
      json({ team: t });
...
http.createServer(router).listen(PORT, () => {
  console.log(`\n✅  PM Hub running → http://localhost:${PORT}`);
  if (!API_KEY) console.warn('⚠️   ANTHROPIC_API_KEY not set — AI features will return a placeholder message.\n    Export it: export ANTHROPIC_API_KEY=sk-ant-...');
